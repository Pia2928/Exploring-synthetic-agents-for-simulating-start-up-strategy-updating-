...
CONSTRUCTION_TIERS = {
    "trait": {
        "label": "Tier 1: Persona",
        "description": "Built from trait scales -- demographics, alignment strategy, "
                        "founder identity blend, cognitive dials. Good for isolating "
                        "one theoretical variable at a time across a set of agents.",
    },
    "interview": {
        "label": "Tier 2: Idiographic",
        "description": "Grounded in real interview material from an actual founder. "
                        "The alignment strategy, identity blend, and cognitive dials "
                        "become secondary, optional scaffolding rather than the "
                        "primary source of the agent's voice and reasoning.",
    },
}
...
def new_agent(
    ...
    return {
        ...
        "name": name,
        "tier": tier if tier in CONSTRUCTION_TIERS else "trait",
...
def render_qualities_block(agent, include_memory=True):
    ...
    tier = agent.get("tier", "trait")

    if tier == "interview":
        # Tier 2 -- idiographic. The interview material IS the person; the
        # trait scales are optional interpretive scaffolding underneath it,
        # not the primary source of voice or reasoning.
        return f"""DEMOGRAPHICS: {demo_line}

REAL INTERVIEW MATERIAL -- this is grounded in an actual founder. Treat everything below \
as authoritative fact about who you are. Draw your voice, phrasing, and reasoning directly \
from it rather than from the secondary scaffolding further down:
Background / interview notes: {g.get('background') or 'not specified'}
Domain expertise: {g.get('expertise') or 'not specified'}
Prior outcomes: {g.get('outcomes') or 'not specified'}
Stated values / voice (use this to match how they actually talk): {g.get('values') or 'not specified'}

SECONDARY SCAFFOLDING (only use these to fill gaps the interview material doesn't cover; \
if they ever conflict with the interview material above, the interview material wins):
Alignment strategy: "{strat['label']}" ({strat['focus']}) -- mitigates {strat['mitigates']}, \
structurally weak on {strat['blind_spot']}
Founder identity: {identity_line}
Cognitive dials: risk {a['risk']}/10, confirmation bias {a['bias']}/10, group dominance \
{a['dominance']}/10, optimism {a['optimism']}/10
{note_line}
{memory_block}"""
